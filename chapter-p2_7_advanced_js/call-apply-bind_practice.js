...
function sumEvenArguments(){
	...
	return myArgs.reduce(function(acc, next){
		if(next % 2 === 0) {
			acc += next;
		}
		return acc;
...
Examples:
    ...
    bindFn2('greenarguments') // "Elie's favorite color is blue"
...
function bind(fn, thisArg){
	...
	return function() {
		return fn.apply(thisArg, outerArgs.concat([].slice.call(arguments)));
	}
}
...
function flip(fn, thisArg){
	...
	return function() {
		return fn.apply(thisArg, outerArgs.concat([].slice.call(arguments)).slice(0, fn.length).reverse());
	}
}
